...
#ifdef _WIN64
// Win64：使用三级基数树
#define SPAN_MAP_BITS (64 - PAGE_SHIFT)
using SpanMap = TCMalloc_PageMap3<SPAN_MAP_BITS>;
#elif _WIN32
// Win32：使用二级基数树
#define SPAN_MAP_BITS (32 - PAGE_SHIFT)
using SpanMap = TCMalloc_PageMap2<SPAN_MAP_BITS>;
//class SpanMap {
//public:
...
//	}
//	void erase(PAGE_ID k) {
//		std::lock_guard<std::mutex> lock(_mutex);
//		_map.erase(k);
//	}
//private:
//	mutable std::mutex _mutex;
//	std::unordered_map<PAGE_ID, Span*> _map;
//};
#else
// 其他平台默认用 PageMap1（32位兼容）
#define SPAN_MAP_BITS (32 - PAGE_SHIFT)
using SpanMap = TCMalloc_PageMap1<SPAN_MAP_BITS>;
#endif
...
private:
	...
	SpanMap _idSpanMap;  // 统一使用 SpanMap
