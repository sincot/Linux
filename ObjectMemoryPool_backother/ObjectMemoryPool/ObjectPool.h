...
public:
	T* New()	// 获取内存池中的内存空间
	{
		...
		else
		{
			...
			//	_memory = (char*)SystemAlloc(MALLOC_SIZE >> PAGE_SHIFT);	// 优化
			...
			if (_remainBytes < objSize) {
				// 计算需要的页数
				size_t allocSize = (objSize > MALLOC_SIZE) ? objSize : MALLOC_SIZE;
				size_t pages = (allocSize + (1 << PAGE_SHIFT) - 1) >> PAGE_SHIFT;

				_memory = (char*)SystemAlloc(pages);
				if (_memory == nullptr) throw std::bad_alloc();
				_remainBytes = pages << PAGE_SHIFT;
			}
...
private:
	...
	std::mutex _mutex;					// 加上锁，保护 _freeList 和 _memory
};
