...
namespace Log
{
    ...
    private:
        std::string CreateNewFile()   // 基于 max_fsize 和 cur_fsize 的大小来判断 cur_fsize 超过 max_fsize 则创建新文件
        {
            // 获取系统时间，以时间来构造文件的扩展名
            ...
            */
            ...
            // filename << _basename;
            // filename << "-";
            // filename << (s_tm.tm_year + 1900);   // 年
            // filename << (s_tm.tm_mon + 1);    // 月
            // filename << s_tm.tm_mday;   // 日
            // filename << s_tm.tm_hour;   // 时
            // filename << s_tm.tm_min;    // 分
            // filename << s_tm.tm_sec;    // 秒
            // filename << ".log";         // 文件的后缀
