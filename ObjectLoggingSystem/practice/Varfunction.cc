...
//     if(ret != -1 && strp != nullptr)
//     {
//         std::cout << strp << std::endl;
//     }
//     free(strp);     // 即便失败，free(nullptr) 也是安全的
//     va_end(ap);     // 将 ap 置空
// }
...
//     myPrintf(std::forward<Args>(args)...);  // args 为空时自动调用 myPrintf()
// }
...
//     static Singleton* instance_;    // 指针，需要手动管理
...
// {
// public:
//     std::shared_ptr<Animal> GetAnimal(const std::string& name) override  { return nullptr; }
...
// {
// public:
//     std::shared_ptr<Fruit> GetFruit(const std::string& name)  override  { return nullptr; }
...
// protected:      // 子类可以访问父类的成员变量，外部不能访问
...
// {
// public:
//     MacBookBuilder():_computer(new MacBook()) {}    // 创建一台空的 MacBook
...
// private:
//     std::shared_ptr<Computer> _computer;        // 正在组装的电脑
// };

// /* 定义固定的组装流程，不关心具体造什么电脑 */
...
// {
//     // 用智能指针管理 builder
//     std::unique_ptr<Builder> builder = std::make_unique<MacBookBuilder>();  /* 创建建造者，内部同时 new MacBook()（空电脑） */
//     Director director(builder.get());   /* Director 借用 Builder 指针，准备指挥组装 */
//     director.Construct("华硕", "三星显示器");   /* 按固定流程组装 */
//     std::shared_ptr<Computer> computer = builder->GetBuilder();     /* 获取成品 */
//     computer->Print();
