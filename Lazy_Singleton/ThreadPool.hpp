...
namespace THREAD_POOL
{
    ...
    public:
        static ThreadPool<T> *LazySingle() // 获取唯一的单例对象
        {
            if(nullptr == _lazy_singletion)
            {
                LockGuard lockguard(_lock); // 保证多线程访问单例时的单例的安全
                // 若内部单例指针为空，说明是第一次创建对象
                if (nullptr == _lazy_singletion)
                {
                    _lazy_singletion = new ThreadPool<T>(); // 使用构造函数初始化
                    _lazy_singletion->StartThreadPool();    // 启动线程池
                    LOG(LogLevel::INFO) << "first using threadpool and create object";
                }
            }
            return _lazy_singletion;
        }
    ...
    private:
        ...
        static ThreadPool<T> *_lazy_singletion; // 添加单例模式
        static Mutex _lock;                     // 保证创建进程池对象的安全
    };

    // 在类外初始化
    template <class T>
    ThreadPool<T> *ThreadPool<T>::_lazy_singletion = nullptr;
