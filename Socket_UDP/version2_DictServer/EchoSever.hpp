...
using callback_t = std::function<std::string(std::string)>;     // 返回值为 string，参数类型也为 string
...
public:
    ...
    UdpServer(callback_t cb, uint16_t port = default_port): _port(port), _sockfd(default_fd), _cb(cb)
    {}
    ...
    void RunUdpServer()     // 启动服务器
    {
        ...
        while(true)
        {
            ...
            if(n > 0)
            {
                ...
                // 2. 将接收到的数据交给上层
                std::string result = _cb(inbuffer);     // 将单词交给上层处理
                ...
                sendto(_sockfd, result.c_str(), result.length(), 0, (struct sockaddr *)&peer, len);
            }
            else
            {
