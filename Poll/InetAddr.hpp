...
public:
    ...
    InetAddr(const struct sockaddr_in &address) :_address(address), _len(sizeof(address))
    {
        ...
        // inet_ntop: const char *inet_ntop(int af, const void *restrict src,
        //                  char dst[restrict .size], socklen_t size)
        char ipstr[32];     // 缓冲区
        inet_ntop(AF_INET, &(_address.sin_addr), ipstr, sizeof(ipstr));     // 将4字节ip转成字符串ip
        _ip = ipstr;    // 转化成功的 ip 在缓冲区 ipstr 中
